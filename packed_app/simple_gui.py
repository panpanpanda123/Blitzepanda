...
class DataManagementGUI:
    ...
    def setup_download_tab(self):
        ...
        # 创建数据库按钮
        self.create_db_btn = tk.Button(control_frame, text="创建数据库", command=self.create_database,
                              bg='#2980b9', fg='white', font=('Microsoft YaHei', 12, 'bold'),
                              relief='flat', padx=20, pady=8)
    ...
    def create_database(self):
        try:
            # Connect to MySQL server (assumes local installation)
            conn = pymysql.connect(host='localhost', user='root', password='xpxx1688')
            cursor = conn.cursor()
            
            # Create database if not exists
            cursor.execute("CREATE DATABASE IF NOT EXISTS dianping DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci")
            conn.commit()
            conn.select_db('dianping')
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS operation_data (
                    ...
                    `日期` DATE NOT NULL,
                    `美团门店ID` VARCHAR(50) NOT NULL,
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    INDEX idx_date (`日期`),
                    INDEX idx_store_id (`美团门店ID`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
            """)
            
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS cpc_hourly_data (
                    id BIGINT AUTO_INCREMENT PRIMARY KEY,
                    `日期` DATE NOT NULL,
                    `美团门店ID` VARCHAR(50) NOT NULL,
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            ...
            # Create store_mapping table and insert data from CSV
            cursor.execute("""
            ...
            # Load data from store_mapping.csv (assumed in config/ relative to EXE)
            csv_path = Path(__file__).parent / 'config' / 'store_mapping.csv'
            if csv_path.exists():
                with open(csv_path, 'r', encoding='utf-8') as f:
                    ...
                    for row in reader:
                        if row and len(row) == 2:
                            store_name, store_id = row
                            cursor.execute(
                                "INSERT INTO store_mapping (store_name, store_id) VALUES (%s, %s)",
                                (store_name.strip(), store_id.strip())
                            )
            else:
                self.log_message("⚠️ store_mapping.csv not found, skipping data insertion")
            
            conn.commit()
            self.log_message("✅ Database and tables created successfully, including store_mapping data!")
