...
def process_operation_folder(OPERATION_FOLDER):
    ...
    import_to_mysql(df_basic, "operation_data", conn_str, dtype=dtype_op)
...
def process_cpc_folder(CPC_HOURLY_FOLDER):
    ...
    os.makedirs(CPC_HOURLY_FOLDER, exist_ok=True)
    engine = create_engine(f"mysql+pymysql://{db_config['user']}:{db_config['password']}@{db_config['host']}:{db_config['port']}/{db_config['database']}?charset={db_config['charset']}")
    conn_str = f"mysql+pymysql://{db_config['user']}:{db_config['password']}@{db_config['host']}:{db_config['port']}/{db_config['database']}?charset={db_config['charset']}"
    engine = create_engine(conn_str)
    ...
    import_to_mysql(df_all, "cpc_hourly_data", conn_str, dtype=dtype_cpc)
